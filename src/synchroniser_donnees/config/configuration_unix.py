"""Module de configuration pour les systèmes unix"""

import os.path

from synchroniser_donnees.config import configuration

class ConfigurationUnix(configuration.Configuration):
    """Gère la configuration pour les systèmes unix"""

    def __init__(self):
        super().__init__()
        self._chemin = os.path.expanduser("~/.config/outils-req/synchroniser-donnees/config.json")
