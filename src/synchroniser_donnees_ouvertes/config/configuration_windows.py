"""Configuration pour les systèmes Windows"""

import os.path

from . import Configuration

CONFIG_DEFAUT_WINDOWS = """{
    "OutilsREQ": {
...
class ConfigurationWindows(Configuration):
    """Gère la configuration pour les systèmes unix"""

    def __init__(self):
        super().__init__()
        self._chemin = os.path.expanduser(
            "~/AppData/Roaming/outils-req/synchroniser-donnees-ouvertes/config.json"
        )
    ...
    def _creer_config(self, chemin: str):
        ...
        with open(chemin, "w", encoding="UTF-8") as fichier:
            fichier.write(CONFIG_DEFAUT_WINDOWS)
